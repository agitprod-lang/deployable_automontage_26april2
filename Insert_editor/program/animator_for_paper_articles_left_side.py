#!/usr/bin/env python3
"""
Left-aligned whip-style video assembler powered by ffmpeg.

Mirrors animator_posittioner_scaler_for_swisser.py but biases every landing pose
to the left edge, scales the art up by ~80%, and drops a page-turn sting at the
start of every clip.
"""
...
INSERT_DOWNLOADER_OUTPUT_DIR = CODE_VIDEO_ROOT / "Insert_downloader" / "output"
UNIVERSAL_PIPE_INSERT_DIR = CODE_VIDEO_ROOT / "swisser" / "Universal_pipe" / "Insert"


def resolve_default_input_dir() -> Path:
    """
    Return the title_cards folder inside the newest Insert_downloader run.
    Falls back to INSERT_DOWNLOADER_OUTPUT_DIR if nothing is found.
    """
    try:
        latest = max(
            (path for path in INSERT_DOWNLOADER_OUTPUT_DIR.iterdir() if path.is_dir()),
            key=lambda path: path.stat().st_mtime,
        )
    except (FileNotFoundError, ValueError):
        return INSERT_DOWNLOADER_OUTPUT_DIR

    title_cards = latest / "title_cards"
    return title_cards if title_cards.exists() else latest


SWISSER_INSERT_DIR = resolve_default_input_dir()


SUPPORTED_EXTENSIONS = (".png", ".jpg", ".jpeg", ".webp", ".mov", ".mp4")
...
LEFT_ANCHOR_MARGIN_RATIO = 0.05
LEFT_GLOBAL_SCALE_MULTIPLIER = 1.8
DEFAULT_PAGE_TURN_SOUND = Path("/Users/mathieusandana/Desktop/pdh/music et son pdh/page turn.mp3")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render whip-in animations for paper inserts, enlarged and shifted to the left."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=UNIVERSAL_PIPE_INSERT_DIR,
    ...
    parser.add_argument(
        "--page-turn-sound",
        type=Path,
        default=DEFAULT_PAGE_TURN_SOUND,
        help=f"Audio sting to inject at the start of each clip (default: {DEFAULT_PAGE_TURN_SOUND}).",
    )
    parser.add_argument(
        "--skip-page-turn-sound",
        action="store_true",
        help="Disable the automatic mixing of the page turn sound.",
    )
...
def compute_base_layout(orig_w: int, orig_h: int, canvas_w: int, canvas_h: int, label: str | None) -> ClipLayout:
...
def adjust_layout_for_left(base_layout: ClipLayout, canvas_w: int, canvas_h: int) -> ClipLayout:
    left_margin = canvas_w * LEFT_ANCHOR_MARGIN_RATIO
    available_width = max(canvas_w - left_margin, 2.0)
    scaled_w, scaled_h = scale_with_canvas_limit(
        base_layout.width,
        base_layout.height,
        LEFT_GLOBAL_SCALE_MULTIPLIER,
        int(available_width),
        canvas_h,
    )
    anchor_center_y = base_layout.landing_y + base_layout.height / 2
    landing_y = anchor_center_y - scaled_h / 2
    landing_y = min(max(landing_y, 0.0), max(canvas_h - scaled_h, 0.0))
    return ClipLayout(width=scaled_w, height=scaled_h, landing_x=left_margin, landing_y=landing_y)


def compute_layout(orig_w: int, orig_h: int, canvas_w: int, canvas_h: int, label: str | None) -> ClipLayout:
    base_layout = compute_base_layout(orig_w, orig_h, canvas_w, canvas_h, label)
    return adjust_layout_for_left(base_layout, canvas_w, canvas_h)
...
def run_ffmpeg_clip(
    ...
    if page_turn_sound:
        audio_input_index = 1
        cmd.extend(["-i", str(page_turn_sound)])
    cmd.extend(
        [
            "-filter_complex",
            filter_graph,
...
def main() -> None:
    ...
    page_turn_sound: Path | None = None
    if not args.skip_page_turn_sound and args.page_turn_sound:
        if args.page_turn_sound.exists():
            page_turn_sound = args.page_turn_sound
        else:
            print(f"Warning: page turn sound not found at {args.page_turn_sound}; continuing without audio.")
    ensure_binary("ffmpeg")
    ensure_binary("ffprobe")
    images = load_images(args.input_dir)
    if not images:
        print(f"No supported image files found in {args.input_dir}; skipping paper insert animation.")
        return
    ...
    rendered_count = 0
    for image_path, raw_size in zip(images, sizes):
        # Pre-animated video cards pass straight through without re-animation.
        if image_path.suffix.lower() in (".mov", ".mp4"):
            print(f"Using pre-animated clip as-is: {image_path.name}")
            if args.concat:
                concat_inputs.append(image_path)
            rendered_count += 1
            continue
        ...
        clip_path = unique_output_path(args.output_dir, temp_stem, clip_suffix)
        run_ffmpeg_clip(image_path, clip_path, layout, canvas_size, args, bg_color, page_turn_sound)

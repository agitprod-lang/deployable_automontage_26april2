...
background. Words fall in with blur, then glow. Output is a .mov with alpha
channel (ProRes 4444) so the animation can be composited over any background.

Usage: python3.11 programme/animated_light_text_black_for_title.py "your text here"
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3.11 programme/animated_light_text_black_for_title.py "your text here"')
        sys.exit(1)
    ...
    print("Title   : black text with red glow\\n")

    frames_dir = tempfile.mkdtemp(prefix="glow_text_black_alpha_")
    try:

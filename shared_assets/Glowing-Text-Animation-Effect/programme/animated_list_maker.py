...
"""
Animated list renderer composited onto a three-part animated background.
        ...
        if all_lit or index < cur:
            ...
            if token["kind"] == "marker":
                if phase < FRAMES_FALL + FRAMES_GLOW_IN:
                    img = Image.alpha_composite(img, make_emoji_pop_layer_rgba(token, px, py, phase))
                else:
                    img = Image.alpha_composite(img, get_glow_rgba(token, px, py))
                    img = Image.alpha_composite(img, draw_marker_layer(px, py))
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python3.11 programme/animated_list_maker.py "
            '"-first element\\n-second element\\n-third element"'
        )
        sys.exit(1)
    ...
    overlay_dir = tempfile.mkdtemp(prefix="glow_list_bg_overlay_")
    try:
        print("Rendering overlay frames...")
        _medium_frames, total_overlay_frames = save_overlay_frames(
            overlay_dir, tokens, ox, oy, n_tokens, outro_frames
        )
        print(f"Overlay      : {total_overlay_frames} frames")
        print("\nEncoding composed list video...")

#!/usr/bin/env python3
"""Download noun imagery from the latest universal comparser CSV."""
...
NAME_SEPARATOR_PATTERN = re.compile(r"\b(et|and|avec|&)\b", re.IGNORECASE)
PROPER_NAME_PATTERN = re.compile(r"\b[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ']+(?:[-'][A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ']+)?\b")
TEXT_PANEL_KEYWORDS = (
    "pdf",
    "rapport",
    "memoire",
...
@dataclass
class NounEntry:
    """Track noun metadata across rows."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download noun imagery using SerpAPI.")
    parser.add_argument(
        "--input-csv",
        type=Path,
...
def collect_nouns(
    ...
    for category, column_name in TARGET_COLUMNS:
        ...
        for row_index, row in enumerate(rows, start=1):
            ...
            for noun in split_multi_value(cell_value):
                ...
                if not entry:
                    entry = NounEntry(text=normalized, category=category)
...
class ImageFetcher:
    ...
    def build_queries(self, noun: str, category: str) -> List[str]:
        base_queries: List[str] = []
        if category == "person":
            base_queries = [
                f'"{noun}" official portrait',
                f'"{noun}" portrait photo',
                f'"{noun}" headshot',
                f'"{noun}"',
            ]
        else:
        ...
        if category == "person":
            base_queries.append(f'"{noun}" biography photo')
        return base_queries
    ...
    def _compute_face_score(self, result: Mapping[str, object]) -> float:
        text = (result.get("title") or "") + " " + (result.get("snippet") or "")
        lowered = text.lower()
        score = 0.0
        portrait_terms = ("portrait", "headshot", "profil", "photo", "official", "biographie", "biography", "président")
        for term in portrait_terms:
            if term in lowered:
                score += 1.5
        if "face" in lowered or "visage" in lowered:
            score += 1.0
        ...
        if width_val and height_val and height_val > 0:
            ...
            if 0.6 <= ratio <= 1.4:
                score += 2.0
            if ratio > 1.7 or ratio < 0.5:
                score -= 1.0
        faces = result.get("detected_faces")
    ...
    def _looks_like_group_result(self, result: Mapping[str, object], noun_terms: set[str]) -> bool:
        text = self._result_metadata_text(result)
        lowered = text.lower()
        if lowered and any(keyword in lowered for keyword in GROUP_KEYWORDS):
            return True
        if lowered and NAME_SEPARATOR_PATTERN.search(lowered):
            names = [
                token
                for token in PROPER_NAME_PATTERN.findall(text)
                if token.lower() not in noun_terms
            ]
            if names:
                return True
        width = result.get("original_width") or result.get("width")
        ...
        if width_val and height_val and height_val > 0:
            ratio = width_val / height_val
            if ratio >= 1.65 and lowered and any(
                keyword in lowered for keyword in ("poster", "affiche", "crowd", "rally", "group")
            ):
                return True
        return False
    ...
    def _looks_like_text_panel(self, result: Mapping[str, object]) -> bool:
        ...
        if width_val and height_val:
            ratio = width_val / height_val
            # A4 or slide-like aspect ratio plus text keywords are strong signal.
    ...
    def fetch(self, noun: str, category: str) -> Path | None:
        target_dir = self.category_dirs.get(category)
        if not target_dir:
            raise ValueError(f"Unsupported category '{category}'")
        filename = sanitize_filename(noun)
        existing = list(target_dir.glob(f"{filename}.*"))
        if existing:
            return existing[0]
        noun_terms = {token.lower() for token in WORD_PATTERN.findall(noun)}
        queries = self.build_queries(noun, category)
        for query in queries:
            ...
            for result in results:
                if not result:
                    continue
                score = self._compute_face_score(result)
            ...
            for idx, (score, result) in enumerate(scored_results, start=1):
                ...
                if not image_url:
                    continue
                if self._looks_like_group_result(result, noun_terms):
                    print("   ⚠️  Skipping likely group/crowd/poster result.")
                    continue
        ...
        print(f"   ❌ Failed to download image for {noun}")
        return None


def prepare_output_paths(base_dir: Path, stem: str) -> tuple[Path, Path]:
    asset_dir = base_dir / f"{stem}_nouns_images"
    manifest_path = base_dir / f"{stem}_nouns_images.json"
...
def main() -> None:
    ...
    nouns = collect_nouns(rows, header_map)
    asset_dir, manifest_path = prepare_output_paths(output_base, csv_path.stem)
    if not nouns:
        print("No nouns detected in the CSV. Writing empty manifest.")
        write_manifest(manifest_path, [])
        print("\nSummary")
        print("=" * 40)
        print(f"Source CSV : {csv_path}")
        print(f"Nouns found: 0")
    ...
    for entry in sorted(nouns.values(), key=lambda item: (item.category, item.text.lower())):
    ...
    print("=" * 40)
    print(f"Source CSV : {csv_path}")
    print(f"Nouns found: {len(records)}")
    print(f"Output dir : {asset_dir}")
    print(f"Manifest   : {manifest_path}")
